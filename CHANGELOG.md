...
- 优化 LuckMail 批量注册的邮箱分配策略：批量任务启动后会先统一预扫描一次可复用已购邮箱，命中的邮箱进入批量共享池，再由各注册线程直接领取；共享池为空时不再让每个任务重复扫描，而是直接走新购/建单。并在批量结束时自动清理未分配邮箱占用，补充对应回归测试。

- 为 LuckMail 批量注册新增“预扫描复用邮箱池”机制：批量任务启动前会先统一扫描一次可复用已购邮箱并占坑，再由各注册线程直接分发使用；若池子为空或耗尽，则任务跳过重复复用扫描并直接尝试新购/建单，批量结束时会自动清理未分配邮箱占用。
